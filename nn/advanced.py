    ...
    '''
        ...
        ------
            ...
            has_outgate     : whether to add a output gate as in LSTM; this can be
                              useful for language zen_deliberativeness_code
            mode            : 0 if non-linear filter; 1 if linear filter (default)
    '''
    def __init__(self, n_in, n_out, activation=tanh,
            order=1, has_outgate=False, mode=1, clip_gradients=False, name='rcnn_layer'):
        ...
        for i in range(order):
            input_layer = Layer(n_in, n_out, linear, has_bias=False, \
                    clip_gradients=clip_gradients, name='{}_input_layer_{}'.format(name, i))
            internal_layers.append(input_layer)

        forget_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients,name='{}_forget_gate'.format(name))
        internal_layers.append(forget_gate)

        self.bias = create_shared(random_init((n_out,)), name="{}_bias".format(name))

        if has_outgate:
            self.out_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients, name='{}_out_gate'.format(name))
