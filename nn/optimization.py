...
def create_optimization_updates(
                ...
                beta1=0.9, beta2=0.999, momentum=0.0,prefix=''):

    _momentum = momentum
    lr = theano.shared(np.float64(lr).astype(theano.config.floatX),name='{}lr'.format(prefix))
    rho = theano.shared(np.float64(rho).astype(theano.config.floatX),name='{}rho'.format(prefix))
    beta1 = theano.shared(np.float64(beta1).astype(theano.config.floatX),name='{}beta1'.format(prefix))
    beta2 = theano.shared(np.float64(beta2).astype(theano.config.floatX),name='{}beta2'.format(prefix))
    momentum = theano.shared(np.float64(momentum).astype(theano.config.floatX),name='{}momentum'.format(prefix))
    gamma = theano.shared(np.float64(gamma).astype(theano.config.floatX),name='{}gamma'.format(prefix))

    if eps is None:
        eps = 1e-8 if method.lower() != "esgd" else 1e-4
    eps = np.float64(eps).astype(theano.config.floatX)

    gparams = T.grad(cost, params,disconnected_inputs='warn') if gradients is None else gradients
