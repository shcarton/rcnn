...
class Layer(object):
    ...
    def initialize_params(self, n_in, n_out, activation):
        ...
        if self.initial_bias:
            b_vals = self.initial_bias * np.ones((n_out,), dtype=theano.config.floatX)
            print 'Setting initial bias value to {} in layer'.format(self.initial_bias)

        self.W = create_shared(W_vals, name=self.name+'_W')
        if self.has_bias: self.b = create_shared(b_vals, name=self.name+'_b')
...
class RecurrentLayer(Layer):
    ...
    '''
    def __init__(self, n_in, n_out, activation,
            clip_gradients=False, name='RecurrentLayer'):
...
class EmbeddingLayer(object):
    ...
    '''
    def __init__(self, n_d, vocab, oov="<unk>", embs=None, fix_init_embs=True, verbose=False):
        ...
        if embs is not None:
            ...
            if verbose: print '{} duplicate words found, including: {}'.format(len(dups),dups[0:10])
            ...
            if verbose: say("{} pre-trained embeddings loaded.\n".format(len(emb_vals)))

            # Randomly initialize the vector for any word not in the vocabulary
            for word in vocab:
                if word not in vocab_map:
        ...
        self.onehot = create_shared(np.identity(self.n_V))
        # self.onehot = create_shared(sparse_identity(self.n_V, format='csc'))
...
class LSTM(Layer):
    ...
    def __init__(self, n_in, n_out, activation=tanh,
            clip_gradients=False, name='LSTM'):
        ...
        self.in_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients,name=name+'_in_gate')
        self.forget_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients,name=name+'_forget_gate')
        self.out_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients,name=name+'_out_gate')
        self.input_layer = RecurrentLayer(n_in, n_out, activation, clip_gradients,name=name+'_input_layer')
...
class GRU(Layer):
    ...
    def __init__(self, n_in, n_out, activation=tanh,
        ...
        self.reset_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients, name=name+'_reset_gate')
        self.update_gate = RecurrentLayer(n_in, n_out, sigmoid, clip_gradients, name=name+'_update_gate')
        self.input_layer = RecurrentLayer(n_in, n_out, activation, clip_gradients, name=name+'_input_gate')
